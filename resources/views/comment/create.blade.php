...
@section('title', 'Блог ЛарА - Добавление комментария')

@section('h1', 'Форма добавления комментария')

@section('content')
    <form action='{{ route('comment.store', ['postId' => $postId]) }}' method="POST">
        @csrf

        <div class="mb-3">
            <label for="commentContent" class="form-label">Содержимое комментария</label>
            <textarea name='commentContent' class="form-control" id="commentContent" aria-describedby="textareaHelp" style='height: 10rem' placeholder="Введите содержимое комментария" required>{{ old('commentContent') }}</textarea>
